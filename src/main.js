window.renderRentViz = basePath => {
  ...
  d3.json(`${basePath}/data/nrw-landkreise.topo.json`, d => {
    const geoData = topojson.feature(d, d.objects.layer)
...
const renderScatter = basePath => {
  ...
  const S = d3.playbooks.scatterChart({
    ...
    getLegendItems: ({data}) => {
      ...
      const population = [100000, 1000000]
...
const renderComparisonMaps = ({geoData, data}) => {
  ...
  const renderMap = (year, legend=true, otherMap=null) => {
    ...
    // else if (otherMap) {
    //   sMap.hilightNode(d => otherMap.hilight(d))
    // }
  ...
  const map2012 = renderMap(2012)
  renderMap(2016, false, map2012)
